...
public class ItemService {
    ...
    public ItemDto create(ItemDto dto, Long userId) {
        ...
        if (dto.getRequestId() != null) {
            item.setRequest(itemRequestRepository.findById(dto.getRequestId()).orElseThrow(() -> new RuntimeException(String.format("Запрос с ID %d не найден", dto.getRequestId()))));
        }

        item = itemRepository.save(item);
        ItemDto resultDto = toItemDto(item);
        resultDto.setRequestId(item.getRequest() != null ? item.getRequest().getId() : null);
        return resultDto;
    }
    ...
    public ItemDto update(Long itemId, ItemDto dto, Long userId) {
        ...
        if (dto.getRequestId() != null) {
            if (!dto.getRequestId().equals(item.getRequest() != null ? item.getRequest().getId() : null)) {
                item.setRequest(itemRequestRepository.findById(dto.getRequestId()).orElseThrow(() -> new RuntimeException(String.format("Запрос с ID %d не найден", dto.getRequestId()))));
            }
        } else if (dto.getRequestId() == null && item.getRequest() != null) {
    ...
    private void validate(ItemDto dto) {
        ...
        if (dto.getRequestId() != null && !itemRequestRepository.existsById(dto.getRequestId())) {
            throw new RuntimeException(String.format("Запрос с ID %d не найден", dto.getRequestId()));
        }
    }
}